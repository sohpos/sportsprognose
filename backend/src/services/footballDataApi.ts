...
async function mapApiMatch(m: any, isPast: boolean = false): Promise<Match> {
  ...
  return {
    id: isPast ? `past-${m.id}` : `live-${m.id}`,
    leagueId: m.competition?.code || 'UNK',
    leagueName: m.competition?.name || 'Unknown League',
    utcDate: m.utcDate,
    status: isPast ? 'FINISHED' : (m.status === 'TIMED' ? 'SCHEDULED' : m.status),
    homeTeam: {
      id: String(homeId),
...
export async function getUpcomingMatches(league?: string): Promise<Match[]> {
  ...
  try {
    ...
    const mapped = await Promise.all(allMatches.map(m => mapApiMatch(m, false)));
...
export async function getPastMatches(league?: string): Promise<Match[]> {
  ...
  try {
    ...
    // Get last 2 weeks of finished matches
    const today = new Date();
    ...
    const buckets = await Promise.all(
      targets.map(code => apiGet(`/competitions/${code}/matches?dateFrom=${from}&dateTo=${to}&status=FINISHED`))
    );

    const allMatches = buckets.flatMap(b => b.matches || []);
    const mapped = await Promise.all(allMatches.map(m => mapApiMatch(m, true)));
...
export async function getMatchById(id: string): Promise<Match | undefined> {
  ...
  if (id.startsWith('past-') || id.startsWith('live-')) {
    ...
    try {
      const data = await apiGet(`/matches/${numericId}`);
