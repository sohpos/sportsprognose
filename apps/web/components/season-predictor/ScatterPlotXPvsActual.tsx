...
export function ScatterPlotXPvsActual({ teams, data }: ScatterPlotProps) {
  ...
  return (
    <div className="rounded-xl bg-white dark:bg-neutral-900 p-4 shadow">
      <h3 className="font-semibold mb-3 text-neutral-800 dark:text-neutral-200">xP vs Actual Points</h3>
...
Δ: ${p.surprise >= 0 ? '+' : ''}${p.surprise.toFixed(1)}`}</title>
              ...
              {/* Team label (for top teams) */}
              {(p.xp > 60 || Math.abs(p.surprise) > 8) && (
                <text
                  x={scaleX(p.xp) + 8}
                  y={scaleY(p.actual) + 3}
