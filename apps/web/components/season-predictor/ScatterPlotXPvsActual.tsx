...
export function ScatterPlotXPvsActual({ teams, data }: ScatterPlotProps) {
  const points = teams.map(t => ({
    ...
    logo: t.logo,
    xp: data[t.id]?.xp ?? 0,
    actual: data[t.id]?.actualPoints ?? 0,
    surprise: data[t.id]?.surprise ?? 0,
  })).filter(p => p.xp > 0 && p.actual > 0)

  if (points.length === 0) {
    return (
      <div className="rounded-xl bg-white dark:bg-neutral-900 p-4 shadow-lg">
        <h3 className="font-bold mb-3 text-neutral-800 dark:text-neutral-200">xP vs Actual Points</h3>
  ...
  const scaleY = (v: number) => size - padding - (v / maxVal) * (size - padding * 2)
  ...
  return (
    <div className="rounded-xl bg-white dark:bg-neutral-900 p-4 shadow-lg">
      <h3 className="font-bold mb-4 text-neutral-800 dark:text-neutral-200">xP vs Actual Points</h3>
      ...
      <div className="flex justify-center">
        <svg width={size} height={size} className="overflow-visible">
          ...
          {/* Axes */}
          <line x1={padding} y1={size - padding} x2={size - padding} y2={size - padding} stroke="#9ca3af" strokeWidth="1" />
          <line x1={padding} y1={padding} x2={padding} y2={size - padding} stroke="#9ca3af" strokeWidth="1" />

          {/* Axis labels */}
          <text x={size - padding} y={size - 8} className="text-[10px] fill-neutral-400">xP</text>
          <text x={8} y={padding + 8} className="text-[10px] fill-neutral-400">Actual</text>
          ...
          {points.map(p => {
            ...
            return (
              <g key={p.id}>
                ...
                >
                  <title>{`${p.name}\nxP: ${p.xp.toFixed(1)}\nActual: ${p.actual}\nΔ: ${p.surprise >= 0 ? '+' : ''}${p.surprise.toFixed(1)}`}</title>
                </circle>
      ...
      <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 mt-4 text-xs">
        ...
        <span className="flex items-center gap-1.5">
          ...
          <span className="text-neutral-600 dark:text-neutral-400">Unlucky</span>
        </span>
        <span className="flex items-center gap-1.5">
