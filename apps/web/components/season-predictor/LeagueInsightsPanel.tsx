...
export function LeagueInsightsPanel({ data, teams }: LeagueInsightsPanelProps) {
  const getVolatility = (distribution: number[]) => {
    ...
    const expectedPos =
      distribution.reduce((sum, count, pos) => sum + (pos + 1) * (count / total), 0);
  ...
  const rows = teams
    ...
    .map((t) => {
      ...
      const xgDelta =
        d?.goalsFor !== undefined && d?.xG !== undefined
          ? d.goalsFor - d.xG
          : null;

      const xgaDelta =
        d?.goalsAgainst !== undefined && d?.xGA !== undefined
          ? d.goalsAgainst - d.xGA
          : null;
  ...
  const sortDesc = <T,>(arr: T[], key: keyof T) =>
    [...arr].sort((a, b) => (b[key] as number) - (a[key] as number));

  const sortAsc = <T,>(arr: T[], key: keyof T) =>
  ...
  const Section = ({
    ...
    title: string;
    items: typeof rows;
    formatter: (t: typeof rows[0]) => string;
    color?: 'green' | 'red';
  }) => {
    ...
    return (
      <div className="rounded-lg bg-white dark:bg-neutral-900 p-3 shadow">
        <h3 className="font-bold text-xs mb-2 text-neutral-700 dark:text-neutral-300">
          {title}
        </h3>
        <ul className="space-y-1 text-xs">
          {items.slice(0, 3).map((t, i) => {
            const val = formatter(t);
            ...
            return (
              <li
                key={t.id}
                className="flex justify-between items-center group hover:bg-neutral-50 dark:hover:bg-neutral-800 rounded px-1 -mx-1"
              >
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      <Section
        title="Überperformer"
        items={sortDesc(rows.filter((t) => t.delta !== null), 'delta')}
      ...
      <Section
        ...
        items={sortAsc(rows.filter((t) => t.delta !== null), 'delta')}
        formatter={(t) => t.delta!.toFixed(1)}
        color="red"
      />
      ...
      <Section
        title="Glücksfaktor"
        items={sortDesc(rows.filter((t) => t.luckFactor !== null), 'luckFactor')}
      ...
      <Section
        title="Pechvögel"
        items={sortAsc(rows.filter((t) => t.luckFactor !== null), 'luckFactor')}
        formatter={(t) => `${t.luckFactor!.toFixed(0)}%`}
        color="red"
      />
      ...
      <Section
        title="Stabilste"
        items={sortDesc(rows.filter((t) => t.consistency !== null), 'consistency')}
        formatter={(t) => t.consistency!.toFixed(2)}
      />

      <Section
        title="Volatilste"
        items={sortDesc(rows.filter((t) => t.volatility > 0), 'volatility')}
        formatter={(t) => `±${t.volatility.toFixed(1)}`}
      />

      <Section
        title="xG Offense"
        items={sortDesc(rows.filter((t) => t.xgDelta !== null), 'xgDelta')}
        formatter={(t) =>
          `${t.xgDelta! >= 0 ? '+' : ''}${t.xgDelta!.toFixed(1)}`
        }
        color="green"
      />
      ...
      <Section
        title="xG Defense"
        items={sortAsc(rows.filter((t) => t.xgaDelta !== null), 'xgaDelta')}
      ...
      <Section
        title="Momentum ↑"
        items={sortDesc(rows.filter((t) => t.momentum !== null), 'momentum')}
      ...
      <Section
        title="Momentum ↓"
        items={sortAsc(rows.filter((t) => t.momentum !== null), 'momentum')}
        formatter={(t) => t.momentum!.toFixed(2)}
        color="red"
      />
    </div>
