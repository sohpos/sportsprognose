        ...
        <div className="space-y-1 text-sm max-h-96 overflow-y-auto">
          {championship.length === 0 ? (
            ...
            championship.map((t: any, i: number) => (
              <div key={t.id} className="flex items-center justify-between group hover:bg-neutral-800 rounded px-2 py-1 -mx-2">
                ...
                <span className="font-mono text-green-400 font-medium text-xs">{formatPct(t.prob)}</span>
              </div>
            ))
          )}
        </div>
