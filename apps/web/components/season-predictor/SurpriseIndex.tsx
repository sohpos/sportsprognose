...
export function SurpriseIndex({ data, teams }: SurpriseIndexProps) {
  ...
  const byLuck = rows
    .filter((r) => r.luckFactor !== null)
    .sort((a, b) => (b.luckFactor ?? 0) - (a.luckFactor ?? 0));

  const byConsistency = rows
    .filter((r) => r.consistency !== null)
    .sort((a, b) => (b.consistency ?? 0) - (a.consistency ?? 0));

  const byXG = rows
    .filter((r) => r.xgDelta !== null)
    .sort((a, b) => (b.xgDelta ?? 0) - (a.xgDelta ?? 0));
  ...
  const Card = ({
    ...
    subtitle: string;
    items: typeof rows;
    getValue: (r: typeof rows[0]) => string;
    color?: boolean;
  }) => (
    <div className="rounded-xl bg-white dark:bg-neutral-900 p-4 shadow-lg">
      <h3 className="font-bold text-sm mb-1 text-neutral-800 dark:text-neutral-200">
        {title}
      </h3>
      ...
      <div className="space-y-1.5">
        {items.slice(0, 5).map((r) => {
          ...
          return (
            <div
              key={r.id}
              className="flex justify-between text-xs group hover:bg-neutral-50 dark:hover:bg-neutral-800 rounded px-1 py-0.5 -mx-1"
            >
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <Card
        title="Luck Factor"
        subtitle="Δ-Points als % vom xP"
        items={byLuck}
        getValue={(r) =>
          r.luckFactor !== null
            ? `${r.luckFactor >= 0 ? '+' : ''}${r.luckFactor.toFixed(1)}%`
            : '-'
        }
        color
      />
      ...
      <Card
        title="Consistency"
        subtitle="1/Volatility – höher = stabiler"
        items={byConsistency}
        getValue={(r) =>
          r.consistency !== null ? r.consistency.toFixed(2) : '-'
        }
      />
      ...
      <Card
        ...
        getValue={(r) =>
          r.xgDelta !== null
            ? `${r.xgDelta >= 0 ? '+' : ''}${r.xgDelta.toFixed(1)}`
            : '-'
        }
        color
      />
      ...
      <Card
        title="Momentum"
        subtitle="Form-Trend (letzte 3)"
        items={byMomentum}
        getValue={(r) =>
          r.momentum !== null
            ? `${r.momentum >= 0 ? '↑' : '↓'}${Math.abs(
                r.momentum
              ).toFixed(2)}`
            : '-'
        }
